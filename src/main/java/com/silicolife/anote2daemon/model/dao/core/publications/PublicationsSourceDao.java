package com.silicolife.anote2daemon.model.dao.core.publications;

import com.silicolife.anote2daemon.model.dao.core.GenericDao;
import com.silicolife.anote2daemon.model.pojo.PublicationsSource;

public interface PublicationsSourceDao extends GenericDao<PublicationsSource> {

	public static final Class<PublicationsSource> className = PublicationsSource.class;

}
