...
public final class CustomCookieCsrfTokenRepository implements CsrfTokenRepository {
	...
	private final Method setCookiePathMethod;
	...
	public CustomCookieCsrfTokenRepository() {
		...
		this.setCookiePathMethod = ReflectionUtils.findMethod(Cookie.class, "setPath", String.class);
		if (this.setCookiePathMethod != null) {
	...
	public void saveToken(CsrfToken token, HttpServletRequest request,
		...
		if ((cookiePath == null || cookiePath.isEmpty()) && setCookiePathMethod != null) {
			ReflectionUtils.invokeMethod(setCookiePathMethod, cookie, getCookiePath(request));
		}else if(cookiePath != null && !cookiePath.isEmpty()){
	...
	/**
	 * Sets the CookiePath attribute on the cookie containing the CSRF token.
	 * The cookie will only be marked as HttpOnly if both <code>cookiePath</code> is context path and the underlying version of Servlet is 3.0 or greater.
	 * Defaults to context path if the underlying version of Servlet is 3.0 or greater.
	 * NOTE: The {@link Cookie#setHttpOnly(boolean)} was introduced in Servlet 3.0.
	 *
	 * @param cookiePath context path sets the cookiePath attribute
	 * @throws IllegalArgumentException if <code>setCookiePath</code> is context path and the underlying version of Servlet is less than 3.0
	 */
	public void setCookiePath(String cookiePath) {
		if (cookiePath != null && !cookiePath.isEmpty() && setCookiePathMethod == null) {
			throw new IllegalArgumentException("Cookie will not be marked as CookiePath because you are using a version of Servlet less than 3.0. NOTE: The Cookie#setCookiePath(String) was introduced in Servlet 3.0.");
		}
